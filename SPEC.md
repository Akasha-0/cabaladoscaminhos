...
1. **[LOW] Tunable domain weights (in progress):** `PESOS_TRADICAO_DOMINIO` matrix exists in `mapeamentos/types.ts` with well-documented weights per tradição × domínio. The matrix is hardcoded. Iter42 adds `getTradicaoWeights()` / `setTradicaoWeights()` runtime override API — weights become tunable at runtime without source changes. Database-backed tuning (admin UI + per-user override) remains a future gap.
...
3. **[MEDIUM] Ritual database:** ✅ Persistence implemented (June 2026). `AreaHistoryEntry` now stores `ritualTitle`, `ritualInstruction`, `ritualDuration`, `ritualElement`, `ritualColor` per area per day. `RitualTrendsSection` in `EvolutionPatterns` displays per-area ritual history. Ritual content still generated from hologram data — curated ritual script database remains a future gap.
4. **[MEDIUM] Layer 7 (Agente Evolutivo):** ✅ Fully implemented (iter30–34). `evolutionary-agent/index.ts` (iter30) — `deriveExercisesFromSnapshot`, `deriveCycleModulation`, `trackAreaRead`, `detectAreaPatterns`; persistence layer — `CycleSnapshot`, `AreaHistoryEntry`, `ExerciseCompletion` Prisma models + API routes (iter33); UI — `EvolutionPatterns` component in "Sua Jornada" tab (iter34); `cycle` plumbed from `/api/akasha/daily` to `useAkashaSynthesis` to `AreaCard` and `PriorityAreasQuickView` (iter32). **Remaining gaps:** milestone-based re-assessment triggers, I Ching cycle integration, formal "gêmeo digital simbólico" UI representation.
5. **[LOW] Procedência audit trail:** ✅ Implemented (June 2026, Iter39). `ProcedenciaEntry` type added to `mapeamentos/types.ts`; `deriveTradicao()`, `extractSimbolo()`, `extractProcedenciaTop()` helpers in `mapeamentos/index.ts`; `synthesizePrimitives()` populates `procedenciaTop`; `buildAkashaSynthesis` attaches per-area `procedencia` via `PRIMITIVOS_POR_AREA` heuristic; `ProcedenciaBadge` component in `AkashaLifeAreasDashboard.tsx` renders top-3 entries as gold chips.
