...
| v0.0.6 | **Refatoração Arquitetural + Curadoria Pendente** — `src/lib/` reorganizado em 5 camadas (domain/application/infrastructure/interface/shared); `apps/akasha-portal/tests/` movido para `tests/` (raiz); `src/lib/prisma.ts` movido para `src/lib/infrastructure/prisma.ts`; 2 testes-guardião arquiteturais (`tests/architecture/clean-architecture.test.ts` + `tests/architecture/package-boundaries.test.ts`); curadoria-guardian consolidado (`tests/lib/grimoire/curatorship-guardian.test.ts`); IDEIA.md §7.4-§7.6 placeholders (Odu nascimento, ervas, corpos — deferred to v0.0.7 per AD-20.8); `grimoire:audit` script. **T11-T13 deferred to v0.0.7** (curador humano per AD-20.8). | (v0.0.6) | ✅ |
| Akasha-v3-iter7 | **SynthesizePrimitives Integration (Prioridade 1)** — `synthesizePrimitives` do `@akasha/core` integrado em `buildAkashaSynthesis`; 5 pilar converters (IChing, Cabala, Astrologia, Tantra, Odu); `AkashaSynthesis.synthesizedProfile` adicionado; `derive-area-narratives.ts` 6 funções com `_synthesizedProfile` param; `area-builders.ts` `buildShadowSymptoms`/`buildGiftStrengths` enriquecidos com top-2 primitivos sombra/luz; `synthesizePrimitives` exportado de `@akasha/core`; npm build ✅ + 121 testes ✅ | (este ciclo) | ✅ |
| Akasha-v3-iter11 | **UX Round 12 — 9 CRÍTICAS + 14 HIGH fixes** — Lua shadow, perfilGeral 3 pilares, diário stubBadge, número-base→Caminho de Vida, dimFoco expand, Oráculo use client, Mandala SVG WCAG, Conexões dynamic connection, Dashboard Authority footer; 49/49 ✅ | (este ciclo) | ✅ |
| Akasha-v3-iter14 | **UX Round 14 — Meaning + WCAG + Accessibility** — Akasha archetype framing fixed, accordion affordance "Toque para ver mais" + aria-label + no paragraph repeat, Dashboard Prioridade badge removed, Regra Prática "Quando" prefix, "Explorar →" pill button, Diário stubBadge removed, counter ✗04→✓04, Conexões % + Lucide icon + UUID, Mandala C1 legend + layer WCAG fonts, MandalaNarrative emoji→Lucide, Oráculo char count, DimensaoCard WCAG fonts, middleware Edge Runtime fix; 49/49 ✅ | (este ciclo) | ✅ |
| Rota | Limite | Janela |
|------|--------|--------|
