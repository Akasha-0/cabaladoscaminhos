...
vi.mock('@/lib/prisma', () => {
  ...
  const create = vi.fn();
  const $transaction = vi.fn(async (cb: (tx: typeof txMock) => Promise<unknown>) =>
    cb(txMock)
  );
  return {
    prisma: {
...
describe('schemas (Zod)', () => {
  ...
  it('createConsultationSchema rejects empty readingId', () => {
    expect(() =>
      createConsultationSchema.parse({ readingId: '', operatorId: 'op-1' })
    ).toThrow();
  });
...
describe('createConsultation', () => {
  ...
  it('throws when reading does not exist', async () => {
    ...
    await expect(
      createConsultation({ readingId: 'missing', operatorId: 'op-1' })
    ).rejects.toThrow(/Reading missing não encontrado/);
  });
  ...
  it('throws when operator does not exist', async () => {
    ...
    await expect(
      createConsultation({ readingId: 'r1', operatorId: 'missing' })
    ).rejects.toThrow(/Operator missing não encontrado/);
  });
});
