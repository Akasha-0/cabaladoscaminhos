...
describe('Odú-Ifá Zodíaco Correlation', () => {
  ...
  describe('getAllOduZodiacs', () => {
    ...
    it('should contain valid zodiac signs', () => {
      const result = getAllOduZodiacs();
      const validSigns = ['Áries', 'Touro', 'Gêmeos', 'Câncer', 'Leão', 'Virgem', 'Libra', 'Escorpião', 'Sagitário', 'Capricórnio', 'Aquário', 'Peixes'];
      for (const mapping of result) {
  ...
  describe('getOduByElement', () => {
    it('should return Odús with fogo element', () => {
      ...
      expect(result.length).toBeGreaterThan(0);
      for (const mapping of result) { expect(mapping.elemento).toBe('fogo'); }
    });

    it('should return Odús with água element', () => {
      const result = getOduByElement('água');
      expect(result.length).toBeGreaterThan(0);
      for (const mapping of result) { expect(mapping.elemento).toBe('água'); }
    });

    it('should return Odús with ar element', () => {
      const result = getOduByElement('ar');
      expect(result.length).toBeGreaterThan(0);
      for (const mapping of result) { expect(mapping.elemento).toBe('ar'); }
    });

    it('should return Odús with terra element', () => {
      const result = getOduByElement('terra');
      expect(result.length).toBeGreaterThan(0);
      for (const mapping of result) { expect(mapping.elemento).toBe('terra'); }
    });

    it('should return empty array for unknown element', () => {
      expect(getOduByElement('éter')).toEqual([]);
    });
  ...
  describe('hasOduZodiac', () => {
    ...
    it('should cover all 16 Odu numbers', () => {
      for (let i = 1; i <= 16; i++) { expect(hasOduZodiac(i)).toBe(true); }
      expect(hasOduZodiac(17)).toBe(false);
    });
  });
  ...
  describe('getOduMessage', () => {
    ...
    it('should contain relevant spiritual themes in messages', () => {
      expect(getOduMessage(1)?.toLowerCase()).toContain('coragem');
      expect(getOduMessage(8)?.toLowerCase()).toContain('transforma');
    });
  });
  ...
  describe('Type exports', () => {
    it('should export OdTuZodiac interface', () => {
      const mapping = {
        ...
        signo: 'Áries', elemento: 'fogo', significado_espiritual: 'Test', mensagem_central: 'Test',
      };
  ...
  describe('Spiritual correlation integrity', () => {
    ...
    it('should distribute elements across all Odús', () => {
      ...
      for (let i = 1; i <= 16; i++) { elements.add(ODTU_ZODIAC_MAPPINGS[i].elemento); }
    ...
    it('should have all zodiac signs covered by 16 Odús', () => {
      const signs = new Set();
      for (let i = 1; i <= 16; i++) { signs.add(ODTU_ZODIAC_MAPPINGS[i].signo); }
      expect(signs.size).toBeGreaterThan(0);
    });
