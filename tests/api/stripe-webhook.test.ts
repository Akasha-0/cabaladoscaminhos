...
describe('POST /api/stripe/webhook', () => {
  ...
  it('11. Checkout with enterprise plan enables all modules', async () => {
    ...
    expect(prisma.assinatura.upsert).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        create: expect.objectContaining({ plano: 'enterprise', moduloEmpresa: true }),
      })
    );
  });
  ...
  it('13. Subscription retrieve error → falls back to metadata plano', async () => {
    ...
    expect(response.status).toBe(200);
    expect(prisma.assinatura.upsert).toHaveBeenCalledWith(
      expect.objectContaining({ create: expect.objectContaining({ plano: 'premium' }) })
    );
  });
  ...
  it('19. Route handles unknown price ID gracefully', async () => {
    ...
    expect(prisma.assinatura.update).toHaveBeenCalledWith(
      expect.objectContaining({ where: { userId: 'user_unknown_price' }, data: expect.objectContaining({ plano: 'basico' }) })
    );
  });
