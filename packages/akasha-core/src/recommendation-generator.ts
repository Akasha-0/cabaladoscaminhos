...
export class RecommendationGenerator {
   ...
   */
  async generateFromRAG(query: string, limit: number): Promise<PracticeRecommendationWithConfidence[]> {
  ...
  async generateHybrid(
    ...
    practiceMap.forEach(({ practice, rulesScore, ragScore, rulesReason, ragReason }) => {
      ...
      const combinedScore = 
        HYBRID_WEIGHTS.rules * rulesScore +
        HYBRID_WEIGHTS.rag * ragScore;
