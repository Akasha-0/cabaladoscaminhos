#!/usr/bin/env python3
"""
Akasha Loop Daemon v6 — Autonomous Brain Architecture
===================================================
...
def log(msg: str) -> None:
    print(f"[{datetime.now().strftime('%H:%M:%S')}] DAEMON-v6: {msg}", flush=True)
...
def phase_research(state, memory):
    ...
    log(f"=== RESEARCH v6 (iter {iteration}, intensity={intensity}) ===")
...
def phase_implementation(state, memory):
    iteration = state.get("iteration", 0)
    log(f"=== IMPLEMENTATION v6 (iter {iteration}) ===")
    task_data = _load_json(TASK_FILE, {})
    improvements = task_data.get("improvements", [])
...
# ── IMPLEMENTATION_WAIT (adaptive polling from v5) ──────────────────────────
def wait_implementation(state, memory):
    ...
    # Track absolute elapsed time to detect permanently stuck loops
    loop_start = time.time()
    MAX_WAIT_SECONDS = 300.0   # hard cap: 5 minutes, then force-exit
    ...
    while True:
        ...
        if AGENT_PIDS_FILE.exists():
            for line in AGENT_PIDS_FILE.read_text().strip().split("\n"):
                ...
                if len(parts) == 2:
                    try:
                        pid = int(parts[1])
                        # os.kill(pid, 0) returns True for zombies — check state instead
        ...
        # Detect stale state: restarted mid-cycle without agents ever spawning
        # Also catch the case where agents spawned but died without producing results
        # and we've been waiting long enough that this is clearly a stuck loop
        # Detect stale state: agents died without producing all results
        # Case 1: no results at all after 60s → retry IMPLEMENTATION
        stale_window = 60.0
        if not running and count == 0 and total > 0 and elapsed > stale_window:
            log(f"  WAIT: no agents, no results after {elapsed:.0f}s — retrying IMPLEMENTATION")
            state["phase"] = "IMPLEMENTATION"
            save_state(state)
            return False
        # Case 2: partial results + all agents dead + stalled → proceed with what we have
        stalled_window = 90.0  # if no new results in 90s after all agents died
        if not running and 0 < count < total and elapsed > stalled_window:
            log(f"  WAIT: partial results ({count}/{total}), all agents dead, stalled {elapsed:.0f}s — proceeding with {count} result(s)")
            break

        # Hard timeout: prevent permanent hang regardless of other conditions
        if elapsed >= MAX_WAIT_SECONDS:
            log(f"  WAIT: hard timeout after {elapsed:.0f}s ({count}/{total} results) — proceeding")
            break

        # All results collected — proceed to QA immediately (don't wait for agent process cleanup)
        if count >= total:
            break
...
def phase_validation(state):
    iteration = state.get("iteration", 0)
    log(f"=== VALIDATION v6 (iter {iteration}) ===")
...
def phase_release(state, memory):
    iteration = state.get("iteration", 0)
    log(f"=== RELEASE v6 (iter {iteration}) ===")
    ...
    if has_changes:
        ...
        msg = f"iter{iteration:03d}: autonomous evolution v6"
...
def main():
    ...
    log(f"Daemon v6 started PID={os.getpid()} — lazy loading active")
