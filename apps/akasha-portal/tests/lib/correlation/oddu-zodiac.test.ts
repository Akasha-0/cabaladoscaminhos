 ...
 * Tests for direct Odú Ifá to Zodiac sign mappings
 */
...
describe('Odú-Ifá Zodíaco Correlation', () => {
  ...
  describe('ODU_ZODIAC_MAPPINGS constant', () => {
    it('should have 16 entries', () => {
      expect(Object.keys(ODU_ZODIAC_MAPPINGS).length).toBe(16);
    });

    it('should have keys 1-16', () => {
      for (let i = 1; i <= 16; i++) {
        expect(ODU_ZODIAC_MAPPINGS[i]).toBeDefined();
      }
    });

    it('should be frozen', () => {
      expect(Object.isFrozen(ODU_ZODIAC_MAPPINGS)).toBe(true);
    });

    it('should have correct structure for each entry', () => {
      for (let i = 1; i <= 16; i++) {
        const mapping = ODU_ZODIAC_MAPPINGS[i];
  ...
  describe('Spiritual correlation integrity', () => {
    ...
    it('should have valid spiritual messages for all Odús', () => {
      for (let i = 1; i <= 16; i++) {
        const msg = ODU_ZODIAC_MAPPINGS[i].mensagem_central;
        expect(msg).toBeDefined();
