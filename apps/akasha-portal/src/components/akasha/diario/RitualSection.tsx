...
export function RitualSection({ ritual, pilarInfo, locale }: RitualSectionProps) {
  ...
  const explainText =
    ritual.titulo === 'Conta-Cantiga'
      ? t('diario.ritual.explicaContaCantiga')
      : ritual.titulo === 'Respiração do Céu'
      ? t('diario.ritual.explicaRespiracao')
      : ritual.titulo === 'Varredura dos 11'
      ? t('diario.ritual.explicaVarredura')
      : ritual.titulo === 'Oração ao Ori'
      ? t('diario.ritual.explicaOracao')
      : t('diario.ritual.explicaIching');
  ...
  <p className="text-[0.9rem] leading-relaxed text-[#B8BFCE]">{ritual.instrucao}</p>
      ...
      <button
        ...
        className="mt-3 text-[0.72rem] text-[#8A9BB8] hover:text-white/60 transition-colors flex items-center gap-1"
