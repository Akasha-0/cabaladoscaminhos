...
export default function ConexoesClient({ userProfile }: Props) {
  ...
  const [rawData, setRawData] = useState<RawBirthData>({
    name: '',
    birthDate: '',
    birthTime: '',
    birthCity: '',
    ...
    try {
      ...
      if (res.status === 400) {
        ...
        if (msg.includes('Mapa natal não encontrado')) {
          setError(
            <>
              Complete seu mapa primeiro.{' '}
              <a href="/mapa/significado" className="underline text-[#9D86FF]">
                Ir para Mapa →
              </a>
            </>
          );
  ...
  const authColor = result
    ? result.authorityMatch === 'aligned'
      ? '#34d399'
      : result.authorityMatch === 'complementary'
        ? '#fbbf24'
        : '#f87171'
    : '#fff';
    ...
    <div className="min-h-screen bg-[#0B0E1C] text-white px-4 py-8 max-w-xl mx-auto">
      ...
      {stage === 'results' && result && (
        <div className="space-y-6">
          ...
          <div
            role="region"
            aria-label="Síncronia Espiritual"
            className="rounded-2xl border border-white/10 bg-white/5 p-5 space-y-3"
          >
            <h2 className="text-xs text-white/50 uppercase tracking-wider font-semibold">
              Síncronia Espiritual
            </h2>
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-white/50 pl-0.5">
                {result.authorityMatch === 'aligned'
                  ? 'Compatíveis — quando decidem juntos, costumam escolher o mesmo caminho.'
                  : result.authorityMatch === 'complementary'
                    ? 'Complementares — um impulsiona onde o outro hesita.'
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              ...
              <p className="text-xs text-white/50 mt-1">
                {result.bodySync.score >= 71
                  ? 'Alta: sintonia forte — corpos em ressonância, projetos físicos fluem juntos.'
                  : result.bodySync.score >= 41
                    ? 'Média: atenção à vitalidade — cuidem do ritmo de repouso juntos.'
            ...
            >
              ...
              <p className="text-xs text-white/50 mt-1">
                {result.oduSync.score >= 71
                  ? 'Ressonância ancestral intensa — destinos entrelaçados.'
                  : result.oduSync.score >= 41
                    ? 'Ciclos que se cruzam gradualmente — paciência e atenção.'
                    : 'Caminhos ainda se buscam — construam pontes com paciência.'}
              </p>
              {result.oduSync.description ? (
                <p className="text-xs text-white/60 mt-1">{result.oduSync.description}</p>
              ) : null}
              <p className="text-xs text-white/60 mt-1">
                {result.oduSync.sharedOdu
                  ? 'Destino comum — vocações e ciclos compartilhados.'
                  : result.oduSync.complementaryOdu
                    ? 'Caminhos complementares — cada um cobre o que o outro não vê.'
                  ...
                  return (
                    <div key={blockIdx}>
                      ...
                      {sentences.map((sentence: string, i: number) => (
                        <p
                          key={i}
                          className="text-sm text-white/75 leading-relaxed border-l-2 border-[#6350E0]/40 pl-4 mb-1"
                        >
                          {sentence.trim()}
                        </p>
                      ))}
          ...
          {result.recommendations.length > 0 && (
            <div className="rounded-2xl border border-[#2DD4BF]/20 bg-[#2DD4BF]/5 p-5 space-y-3">
              ...
              {result.recommendations.slice(0, 1).map((rec, i) => (
                <div
                  key={i}
                  className="rounded-xl border border-[#2DD4BF]/30 bg-[#2DD4BF]/10 px-4 py-3"
                >
                ...
                <ul className="space-y-1.5 pl-2">
                  {result.recommendations.slice(1).map((rec, i) => (
                    <li key={i} className="text-xs text-white/60 flex gap-2">
                      <span className="text-[#2DD4BF]/60 shrink-0 mt-0.5" aria-hidden="true">
                        •
                      </span>
                      {rec}
                    </li>
          ...
          <div
            role="region"
            aria-label="Orientações de interpretação"
            className="rounded-xl border border-[#6350E0]/20 bg-[#6350E0]/5 p-4"
          >
            <p className="text-xs text-[#6350E0]/80 mb-2 font-semibold">
            ...
            <ul className="space-y-1.5 text-[11px] text-white/60">
              <li className="flex gap-2">
                ...
                Scores altos (71+): reconheça o terreno fértil — aproveite para decisões conjuntas
                importantes.
              </li>
              <li className="flex gap-2">
                <span className="text-[#6350E0]/60 shrink-0">→</span>
                Scores médios (41–70): o campo oscila — ações conjuntas pequenas constroem o
                terreno.
              </li>
              <li className="flex gap-2">
                <span className="text-[#6350E0]/60 shrink-0">→</span>
                Scores baixos (0–40): o contraste é informação — investigue o que cada um busca
                antes de agir juntos.
              </li>
              <li className="flex gap-2">
                <span className="text-[#6350E0]/60 shrink-0">→</span>
                Salve a análise para comparar ao longo do tempo — conexões mudam conforme vocês
                evoluem.
              </li>
            </ul>
          </div>
            ...
            <div className="space-y-3">
              {savedConnections.map((conn) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <p title={conn.otherName} className="font-bold text-white truncate max-w-[60%]">
                      {conn.otherName}
                    </p>
                    <span
