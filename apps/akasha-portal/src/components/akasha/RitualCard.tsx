    ...
    <Card className="w-full max-w-md overflow-hidden">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between gap-2">
          <CardTitle className="text-base font-heading">Ritual do Dia</CardTitle>
          <Badge
            variant="outline"
            style={{
        ...
        <p className="text-xs text-muted-foreground mt-1">{formatDate(ritual.data)}</p>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <section aria-labelledby="ritual-code-title">
          <h3
            id="ritual-code-title"
            className="text-xs font-semibold text-[#7C5CFF] uppercase tracking-wider mb-2"
          >
            Código Akáshico
          </h3>
          <div className="flex items-start gap-4 p-3 rounded-lg bg-[#0B0E1C]/80 border border-[#26304F]/50">
            ...
            <div className="flex flex-col items-center gap-1">
              <div className="flex flex-col-reverse">{renderLinesHexagrama(hexagrama.lines)}</div>
              <span className="text-lg font-bold text-[#A0763A]">{hexagrama.number}</span>
            </div>

            {/* Informações do hexagrama */}
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-sm text-[#F4F5FF] truncate">{hexagrama.name}</p>
              <p className="text-xs text-[#A0763A]">{hexagrama.chineseName}</p>
        ...
        {ritual.pratica && (
          <section aria-labelledby="ritual-practice-title">
            <h3
              id="ritual-practice-title"
              className="text-xs font-semibold text-[#2DD4BF] uppercase tracking-wider mb-2"
            >
              Prática Recomendada
            </h3>
            <div className="p-3 rounded-lg bg-[#0B0E1C]/80 border border-[#26304F]/50">
              <p className="font-semibold text-sm text-[#F4F5FF]">{ritual.pratica.name}</p>
              <p className="text-xs text-[#A7AECF] mt-1">
                {ritual.pratica.tradition} · {ritual.pratica.frequency}
              </p>
              <p className="text-xs text-[#A7AECF] mt-2 leading-relaxed">{ritual.pratica.howTo}</p>
              {ritual.pratica.warnings && ritual.pratica.warnings.length > 0 && (
                <p className="text-xs text-[#FB5781]">
                  <AlertTriangle size={12} className="inline mr-1 flex-shrink-0 text-[#FB5781]" />
        ...
        {ritual.quizilas && ritual.quizilas.length > 0 && (
          <section aria-labelledby="ritual-quizilas-title">
            <h3
              id="ritual-quizilas-title"
              className="text-xs font-semibold text-[#FB5781] uppercase tracking-wider mb-2"
            >
              Quizilás do Odu
            </h3>
            <ul className="space-y-1.5" role="list">
              {ritual.quizilas.map((quizila: Quizila) => (
                ...
                >
                  ...
                  >
                    {quizila.tipo === 'proibicao' ? (
                      <X size={12} />
                    ) : quizila.tipo === 'restricao' ? (
                      <Circle size={12} />
                    ) : (
                      <ArrowRight size={12} />
                    )}
                  </span>
                  <span className="text-xs text-[#A7AECF] leading-relaxed">{quizila.texto}</span>
                </li>
              ))}
            </ul>
          </section>
        ...
        {ritual.oracao && (
          <section aria-labelledby="ritual-prayer-title">
            <h3
              ...
              className="text-xs font-semibold text-[#F0B429] uppercase tracking-wider mb-2"
            >
              Oração do Dia
            </h3>
