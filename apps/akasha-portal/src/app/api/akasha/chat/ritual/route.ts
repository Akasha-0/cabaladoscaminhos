 ...
 * Rota especializada para rituais sugeridos
 * 
 * Força intent='ritual', extrai userCode, chama MentorEngine
 * e retorna apenas o ritual sugerido.
 */
...
const ritualBodySchema = z.object({
  message: z.string().min(1).max(2000).optional().default('Sugira um ritual espiritual para mim'),
...
export async function POST(request: NextRequest) {
  ...
  // 2. Validar body
  let parsed: z.infer<typeof ritualBodySchema>;
  try {
    const raw = await request.json();
    parsed = ritualBodySchema.parse(raw);
  } catch (err) {
  ...
  // 4. Montar ChatRequest forçando intent='ritual'
  ...
  try {
    ...
    // 6. Retornar apenas ritual
    return NextResponse.json({
      ritual: response.ritual ?? null,
    });
  } catch (err) {
    console.error('[chat/ritual/route] Erro no mentor:', err);
