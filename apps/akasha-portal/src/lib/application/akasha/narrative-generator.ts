...
const LIFE_PATH_NARRATIVES: Record<
  number,
  { titulo: string; essencia: string; missao: string; sombra: string; pratica: string }
> = {
  1: {
    titulo: 'Pioneiro',
  ...
  6: {
    ...
    pratica: 'Ofereça 1 ato de cuidado a alguém, e recuse 1 pedido de cuidado a si mesmo.',
  },
  7: {
    titulo: 'Buscador',
  ...
  8: {
    ...
    pratica: 'Liste 3 conquistas suas que não dependeram só de esforço.',
  },
  9: {
    titulo: 'Humanista',
  ...
  11: {
    ...
    missao: 'Confie na primeira impressão. Master-a em algo concreto. Visão sem ação esvai.',
...
function buildAncestralidadeNarrative(kab: KabalisticMap | null, area: string): string {
  if (!kab?.lifePath)
    return 'A Cabala não registrou seu número de vida. O número existe — está por descobrir.';
  const n = reduceMasterNumber(kab.lifePath);
  const lp = LIFE_PATH_NARRATIVES[n];
  if (!lp)
    return `Seu Camino de Vida é ${kab.lifePath}. A interpretação completa ainda está sendo composta.`;
...
function buildMovimentoCelesteNarrative(astro: AstrologyMap | null, area: string): string {
  ...
  const ELEMENT_NARRATIVES: Record<string, Record<string, string>> = {
    fogo: {
      vitalidadeEnergia:
        'Seu corpo é um ponto de luz. Você irradia energia — atenção: não queime os outros com seu brilho. Sua saúde se fortalece com movimento, não com repouso.',
      conexoesAmor:
        'No amor, você ama com intensidade. Seu corpo pede demonstração, não só palavra. Sem expressão, a energia estagna e vira frustração.',
      carreiraProsperidade:
        'Na carreira, você lidera pela visão. Mas fogo sem lenha queima sozinho: certifique-se de que há estrutura ao redor da sua paixão.',
      oriCabecaQuizilas:
        'Sua mente é intuitiva, não analítica. Você sabe antes de entender. Treine confiar na primeira impressão — é sua inteligência mais profunda.',
      missaoDestino:
        'Você veio para inspirar. Sua missão é ser um farol — mas um farol não persegue navios. Permaneça. Os que precisam vão chegar.',
      desafiosSombras:
        'Quando o fogo queima sem propósito, você entra em exaustão. Observe: você está agindo por visão ou por ansiedade? Pausem antes de decisões importantes.',
    },
    terra: {
      vitalidadeEnergia:
        'Seu corpo pede rotina, saúde, contacto com a natureza. A negligência física se manifesta como peso — não só no corpo, na energia. Respeite o instrumento.',
      conexoesAmor:
        'Você ama com presença, não com palavras. Seu corpo se expressa no cuidado diário. A crise surge quando o outro não sente o que você demonstra.',
      carreiraProsperidade:
        'Você constrói para o longo prazo. Mas terra sem água racha: equilibre ambição com diversão. Prosperidade sustentável inclui prazer.',
      oriCabecaQuizilas:
        'Sua mente funciona como arquivo — paciente, organizada. Você sabe quando sabe. Mas saber sem agir é preguiça disfarçada de profundidade.',
      missaoDestino:
        'Você veio para criar estruturas que sustentam outros. Sua missão está nos alicerces que você lança — mesmo que nunca receba crédito.',
      desafiosSombras:
        'Rigidez é sua sombra. Você resiste a mudanças mesmo quando o terreno já mudou. Pergunte-se: estou construindo ou estou preso?',
    },
    ar: {
      vitalidadeEnergia:
        'Seu corpo é instrumento de intercâmbio — isolá-lo gera tensão. Você precisa de movimento, conversa, variação. A quietude é seu inimigo.',
      conexoesAmor:
        'Você ama pela mente. A conexão intelectual é seu pré-requisito. Sem ela, o corpo não responde. Mas só mente sem corpo é zona de amigo.',
      carreiraProsperidade:
        'Você prospera através de ideias, comunicação, conexões. Sua carreira floresce quando envolve outras pessoas — sozinho, a mente dispersa.',
      oriCabecaQuizilas:
        'Sua mente é rápida, versátil, brilhante. O risco é superficialidade: você sabe muito, demora a dominar. Encontre 1 assunto e aprofunde.',
      missaoDestino:
        'Você veio para comunicar. Sua missão é traduzir — o complexo para o simples, o oculto para o acessível. Você é ponte entre mundos.',
      desafiosSombras:
        'Fuga pelo intelecto é sua armadilha. Você pensa quando deveria agir, analisa quando deveria sentir. Pergunte: o que eu estou evitando sentir?',
    },
    água: {
      vitalidadeEnergia:
        'Você sente antes de pensar — seu corpo é receptor. A tensão aparece quando você absorve o que não é seu. Defina fronteiras energéticas.',
      conexoesAmor:
        'Você ama com o corpo inteiro — absorve o outro. Isso é profundo e perigoso. Você precisa aprender o que é seu e o que é do outro.',
      carreiraProsperidade:
        'Você prospera pela intuição, não pela lógica. Seu timing é emocional — quando algo "certa" no ventre, é porque é. Confie mais no corpo.',
      oriCabecaQuizilas:
        'Sua mente é profunda, perceptiva, simbólica. Você entende o que outros só sentem. O risco é ficar na sensação sem traduzir para ação.',
      missaoDestino:
        'Você veio para sentir o que outros não conseguem nomear. Sua missão é ser o receptáculo — guardar o que outros não conseguem segurar. Mas não se perca no que guarda.',
      desafiosSombras:
        'Fuga emocional é sua armadilha. Você analisa para não sentir. Mas o corpo não mente — a emoção reprimida vira sintoma. Sinta primeiro.',
    },
  };

  const base =
    ELEMENT_NARRATIVES[elemento]?.[area] ??
    `Sol${sol ? ' em ' + sol : ''} com ascendente${asc ? ' ' + asc : ''} e planeta dominante${planeta ? ' ' + planeta : ''}. Seu elemento é ${elemento}. A energia deste elemento colore como você opera no mundo.`;
  const luaNote = lua
    ? ` Lua${lua} informa: você ${lua === 'Câncer' ? 'absorve emoções e precisa de fronteira clara.' : lua === 'Escorpião' ? 'sente profundamente e tende a guardar o que sente.' : 'opera pelo perasaanterior ao pensamento.'}`
    : '';
...
function buildCorpoEnergiaNarrative(tantra: TantricMap | null, area: string): string {
  if (!tantra?.soul)
    return 'O Tantra não registrou seu corpo principal. O corpo guarda sabedoria — nomeá-la é primeiro passo.';

  const BODY_MAP: Record<
    number,
    { name: string; essencia: string; mission: string; shadow: string }
  > = {
    1: {
      name: 'Alma',
      essencia: 'Você é puro propósito. Antes do nome, antes do corpo — há uma intenção.',
      mission: 'Descubra o que você veio fazer antes de falar com o mundo.',
      shadow: 'Fugir do mundo antes de concretizar o propósito.',
    },
    2: {
      name: 'Corpo de Desejo',
      essencia: 'O desejo é seu GPS. Ele aponta para o que importa, se você souber ouvi-lo.',
      mission: 'Identifique seu desejo mais profundo — não o imediato, o que sustenta.',
      shadow: 'Ser governado pelo desejo alheio ou pelo medo de querer.',
    },
    3: {
      name: 'Mente Positiva',
      essencia: 'Você pensa em imagens, sons, cheiros. Sua mente é criativa, não linear.',
      mission: 'Dê forma ao que vê: escreva, desenhe, dance, cante.',
      shadow: 'Ficar no plano das ideias sem agir no mundo.',
    },
    4: {
      name: 'Mente Negativa',
      essencia: 'O guardião. Filtra o que entra e o que sai. Sem ele, nada se integra.',
      mission: 'Reconheça o guardião: ele protege, mas às vezes retém demais.',
      shadow: 'Controlar por medo. Reprimir por hábito. Dificuldade de soltar.',
    },
    5: {
      name: 'Corpo Físico',
      essencia: 'O instrumento. Ele é o meio pelo qual a intenção se manifesta no mundo.',
      mission: 'Respeite o instrumento: movimento, descanso, alimentação consciente.',
      shadow: 'Negligenciar o corpo ou obsessão com o corpo como identidade.',
    },
    6: {
      name: 'Corpo de Arco',
      essencia: 'A ponte. Conecta o que você é internamente ao que oferece ao mundo.',
      mission: 'Pergunte-se: o que eu ofereço? Quem se beneficia?',
      shadow: 'Dar demais, receber de menos. Energia em exaustão.',
    },
    7: {
      name: 'Corpo Divino',
      essencia: 'O canal. Atravessa dimensões. Sabe coisas sem saber como sabe.',
      mission: 'Confie na primeira impressão. Pratique a escuta interior.',
      shadow: 'Mediunidade sem filtro. Carregar o que não é seu.',
    },
    8: {
      name: 'Corpo do Campo',
      essencia: 'O campo áurico. Define como você ocupa espaço na presença dos outros.',
      mission: 'Respeite sua presença: você irradia antes de falar.',
      shadow: 'Assumir que todos sentem o que você sente.',
    },
    9: {
      name: 'Corpo de Mahat',
      essencia: 'A mente cósmica. Acesso à sabedoria universal. Meditação é seu idioma.',
      mission: 'Pratique o silêncio. Não para fugir, mas para ouvir mais fundo.',
      shadow: 'Perder-se na abstração. Evitar o concreto.',
    },
    10: {
      name: 'Corpo Radiante',
      essencia: 'O espírito. Quando está ativo, você irradia propósito sem esforço.',
      mission: 'Deixe-se ser visto. Não oculte o que brilha.',
      shadow: 'Arrogar-se mais do que é. Liderança que cega.',
    },
    11: {
      name: 'Corpo Indestrutível',
      essencia:
        'A eternidade. Você sabe, em algum lugar, que nada disso é definitivo — e age mesmo assim.',
      mission: 'Aja com coragem: você já é mais do que este corpo.',
      shadow: 'Usar "tudo é passageiro" para evitar compromisso.',
    },
  };
...
export function buildTransformacaoIChingNarrative(
  ...
  const HEX_NAMES: Record<number, string> = {
    ...
    4: 'Juventude Insensata',
    5: 'Espera',
    6: 'Conflito',
    7: 'O Líder',
    8: 'A União',
    9: 'O Rebanho Menor',
    10: 'Andar com Cautela',
    11: 'Paz',
    12: 'Estagnação',
    13: 'Comunidade de Pessoas',
    14: 'Posse Grande',
    15: 'Modéstia',
    16: 'Entusiasmo',
    17: 'Seguir',
    18: 'Trabalho Corrompido',
    19: 'Aproximação',
    20: 'Contemplação',
    21: 'Mordida Veloz',
    22: 'Graça',
    ...
    50: 'O Caldeirão',
    51: 'O Trovão',
    52: 'A Montanha (Tranquilidade)',
    53: 'O Desenvolvimento',
    54: 'A Moça Casadeira',
    55: 'Abundância (Época)',
    56: 'O Andarilho',
    57: 'A Penetrante (Vento)',
    58: 'A Serena (Lago)',
    59: 'Dispersão',
    60: 'A Limitação',
    61: 'A Verdade Interior',
    62: 'O Pequeno Excesso',
    63: 'Depois da Conclusão',
  ...
  const HEX_AREA_GUIDANCE: Record<string, Record<string, string>> = {
    vitalidadeEnergia: {
      fuego:
        'O hexagrama indica transformação por fogo — sua energia se renova por intensidade, não por repouso.',
      agua: 'O hexagrama indica transformação por água — sua energia flui por adaptação, não por força.',
      terra:
        'O hexagrama indica transformação por terra — sua energia se estabiliza por contacto com o físico.',
    ...
    conexoesAmor: {
      ...
      terra:
        'No amor, o hexagrama aponta para lealdade — você ama construindo, não apenas sentindo.',
    ...
    carreiraProsperidade: {
      ...
      terra:
        'Na carreira, o hexagrama indica construção — você cria valor pela presença, não pela pressa.',
      aire: 'Na carreira, o hexagrama indica comunicação — você prospera por conexões, não por isolamento.',
    },
    oriCabecaQuizilas: {
      fuego:
        'Na mente, o hexagrama traz clareza de fogo — você sabe o que quer antes de entender por quê.',
      agua: 'Na mente, o hexagrama traz profundidade de água — você percebe o que outros só intuem.',
      terra:
        'Na mente, o hexagrama traz solidez de terra — você pensa devagar e fundo, não rápido e raso.',
      aire: 'Na mente, o hexagrama traz velocidade de ar — você conecta ideias antes de ter palavras.',
    },
    missaoDestino: {
      fuego:
        'Na missão, o hexagrama indica criação — você veio para iniciar algo que não existia antes.',
      agua: 'Na missão, o hexagrama indica receptividade — você veio para guardar, não para forçar.',
      terra:
        'Na missão, o hexagrama indica fundamento — você veio para ancorar o que os outros começam.',
  ...
  let primitiveNote = '';
  const ichingPrimitives =
    _synthesizedProfile?.primitivos.filter((p) =>
...
export function generateAreaNarrativeFull(
  ...
  if (astro?.planets?.find((p) => p.planet === 'Sol')?.sign)
    sourceParts.push(`Sol ${astro.planets.find((p) => p.planet === 'Sol')!.sign}`);
  ...
  const SYNTHESIS: Record<string, ParaFn[]> = {
    ...
    missaoDestino: [
      ...
      (k, a, t, o, i?) =>
        ...
        (i
          ? ` O I Ching chama isso de "seguir o momento presente" — ` +
            `quando você age no hexagrama certo, a missão se manifesta sem força.`
          : ` Kabbalah nomeia o destino; ` +
  ...
  if (areaSyntheses) {
    ...
    if (primitiveAnchor) {
      if (hasCount === 1) {
        integratedNarrative = [
          primitiveAnchor + areaSyntheses[0](kabCore, astroCore, tantraCore, oduCore, ichingCore),
          areaSyntheses[3](kabCore, astroCore, tantraCore, oduCore, ichingCore),
        ].join(' ');
      } else if (hasCount === 2) {
        integratedNarrative = [
          primitiveAnchor + areaSyntheses[0](kabCore, astroCore, tantraCore, oduCore, ichingCore),
          areaSyntheses[2](kabCore, astroCore, tantraCore, oduCore, ichingCore),
          areaSyntheses[3](kabCore, astroCore, tantraCore, oduCore, ichingCore),
        ].join(' ');
      } else {
        // hasCount >= 3 but also have primitive anchor: full synthesis
        integratedNarrative = areaSyntheses
          .map((fn) => fn(kabCore, astroCore, tantraCore, oduCore, ichingCore))
          .join(' ');
      }
    } else if (hasCount === 1) {
      integratedNarrative = [
        areaSyntheses[0](kabCore, astroCore, tantraCore, oduCore, ichingCore),
        areaSyntheses[3](kabCore, astroCore, tantraCore, oduCore, ichingCore),
      ].join(' ');
    } else if (hasCount === 2) {
      integratedNarrative = [
  ...
  // Practical example
  const kabPratica = kab?.lifePath
    ? (LIFE_PATH_NARRATIVES[reduceMasterNumber(kab.lifePath)]?.pratica ?? '')
    : '';
  const practicalExample =
    kabPratica.length > 0 && kabPratica.length < 80
      ? kabPratica
...
export function generateSynthesisParagraph(
  ...
  if (typeName) {
    const typeOpening: Record<string, string> = {
      'O Catalisador':
        'Você é O Catalisador — o que inicia antes de estar pronto, que coloca o mundo em movimento só por estar presente',
      'O Receptor':
        'Você é O Receptor — o que capta o que outros emitem, que sabe antes de saber como sabe',
      'O Construtor':
        'Você é O Construtor — o que transforma esforço diário em estruturas que ninguém mais consegue ver ainda',
      'O Transformador':
        'Você é O Transformador — o que atravessa o fogo e ajuda outros a atravessar também',
      ...
      'O Alquimista':
        'Você é O Alquimista — o que vê matéria-prima onde outros veem apenas bloqueio',
  ...
  if (kab?.lifePath) {
    ...
    if (lp) {
      ...
      parts.push(
        `Seu Caminho de Vida é ${kab.lifePath}${masterNote} — ${lp.titulo}. ${lp.essencia}`
      );
    }
  }
  ...
  if (sol) {
    parts.push(
      `Sol em ${sol} (elemento ${elemento}). ${elementoDesc[elemento] ?? ' Seu elemento colore como você sente e reage ao mundo.'}${lua ? ` Lua em ${lua} revela que você precisa de ${lua === 'Câncer' ? 'segurança emocional e fronteiras claras' : lua === 'Escorpião' ? 'intimidade profunda e confiança absoluta' : 'espaço para processar antes de responder'} no dia a dia.` : ''}`
    );
  }
  ...
  if (tantra?.soul) {
    ...
    parts.push(
      `No Tantra, seu ${bodyName} é central. ${bodyEssence[tantra.soul] ?? 'Seu corpo é o instrumento pelo qual tudo se manifesta.'}`
    );
  }
  ...
  if (odu?.oduName) {
    const oduEssence: Record<string, string> = {
      ...
      Oyeku:
        'Oyeku é destruição criativa — você dissolve o que não funciona para que o novo possa nascer.',
    ...
    parts.push(
      `No Odu, ${desc}${odu.elementalForce ? ` Força elemental: ${odu.elementalForce}.` : ''}${odu.orixaRegency?.[0] ? ` Regido por ${odu.orixaRegency[0]}.` : ''}`
    );
  }
