...
import {
  ...
  buildTransformacaoIChingNarrative,
...
function deriveChainOfReasoning(
  ...
  if (area === 'vitalidadeEnergia') {
    if (astro?.dominantPlanet) {
      steps.push(
        `${astro.dominantPlanet} é seu planeta dominante (Akasha) → governa como você canaliza energia`
      );
    }
    if (astro?.elementalChart) {
      ...
      if (dominant && dominant[1] > 0) {
        steps.push(
          `Elemento ${dominant[0]} predominante (${dominant[1]}/4) → seu corpo responde ao mundo por ${dominant[0]}`
        );
      }
    }
    if (tantra?.bodies?.pranic) {
      steps.push(
        `Corpo Prânico ${tantra.bodies.pranic.number}/11 (Energia) → nível de energia vital funcional`
      );
    }
    if (tantra?.bodies?.fisico) {
      steps.push(
        `Corpo Físico ${tantra.bodies.fisico.number}/11 (Energia) → como você habita seu corpo`
      );
    }
    if (kab?.lifePath) {
    ...
    if (odu?.elementalForce) {
      steps.push(
        `Força Elemental ${odu.elementalForce} (Akasha) → o elemento que seu corpo mais precisa nutrir`
      );
    }
    if (steps.length > 0) {
      steps.push(
        `${steps.length} sinais convergentes → Vitalidade é sua área de transformação primária neste ciclo`
      );
    }
    ...
    if (astro?.planets?.find((p) => p.planet === 'Lua')) {
      ...
      steps.push(
        `Lua em ${moon.sign} (Akasha) → seu mundo emocional e estilo de necessidade de cuidado`
      );
    }
    if (tantra?.soul) {
      steps.push(
        `Alma Tântrica ${tantra.soul} (Energia) → seu padrão de vínculo e profundidade emocional`
      );
    }
    if (kab?.lifePath) {
      steps.push(
        `Caminho de Vida ${kab.lifePath} (Akasha) → como você coopera ou compete nos relacionamentos`
      );
    }
    if (odu?.oduName) {
    ...
    if (steps.length > 0) {
      steps.push(
        `${steps.length} sinais convergentes → Conexões é onde sua frequência de transformação se manifesta primeiro`
      );
    }
  } else if (area === 'carreiraProsperidade') {
    if (astro?.midheaven) {
      steps.push(
        `Meio do Céu em ${astro.midheaven} (Akasha) → sua vocação pública e caminho de prosperidade`
      );
    }
    if (kab?.expression) {
    ...
    if (tantra?.divineGift) {
      steps.push(
        `Dom Divino ${tantra.divineGift} (Energia) → talento inato que se expressa no trabalho`
      );
    }
    if (odu?.lifeLesson) {
      steps.push(
        `Lição de Vida ${odu.lifeLesson} (Akasha) → o que você está aqui para aprender e ensinar`
      );
    }
    if (steps.length > 0) {
      steps.push(
        `${steps.length} sinais convergentes → Carreira é sua área de expressão de propósito`
      );
    }
    ...
    if (odu?.orixaRegency?.length) {
      steps.push(
        `Orixá ${odu.orixaRegency[0]} (Akasha) → a energia que governa sua cabeça e decisões`
      );
    }
    if (tantra?.karma) {
      steps.push(
        `Carma Tântrico ${tantra.karma} (Energia) → padrão que precisa ser consciência antes de ação`
      );
    }
    if (kab?.lifePath) {
      steps.push(
        `Caminho de Vida ${kab.lifePath} (Akasha) → o tipo de autoridade que você reconhece`
      );
    }
    ...
    if (steps.length > 0) {
      steps.push(
        `${steps.length} sinais da mente convergem → Ori é sua área de integração do comando`
      );
    }
    ...
    if (kab?.lifePath) {
      steps.push(
        `Caminho de Vida ${kab.lifePath} (Akasha) → o arco de transformação ao longo da vida`
      );
    }
    if (tantra?.destiny) {
    ...
    if (steps.length > 0) {
      steps.push(
        `${steps.length} sinais do destino convergem → Missão é sua área de alinhamento com o propósito maior`
      );
    }
  } else if (area === 'desafiosSombras') {
    if (astro?.planets?.find((p: { planet: string; sign: string }) => p.planet === 'Saturno')) {
      const sat = astro.planets.find(
        (p: { planet: string; sign: string }) => p.planet === 'Saturno'
      )!;
    ...
    if (astro?.planets?.find((p) => p.planet === 'Plutão')) {
      ...
      steps.push(
        `Plutão em ${pl.sign} (Akasha) → seu campo de transformação forçada e regeneração`
      );
    }
    if (kab?.karmicLessons?.length) {
      steps.push(
        `Lições Kármicas ${kab.karmicLessons.join(', ')} (Akasha) → padrões a serem integrados`
      );
    }
    if (kab?.challenges?.main) {
      steps.push(
        `Desafio Principal ${kab.challenges.main} (Akasha) → a sombra que bloqueia seu potencial`
      );
    }
    if (tantra?.karma) {
    ...
    if (odu?.lifeLesson) {
      steps.push(
        `Lição de Vida ${odu.lifeLesson} (Akasha) → o desafio que o Odu coloca para sua evolução`
      );
    }
    if (steps.length > 0) {
      steps.push(
        `${steps.length} sinais da sombra convergem → Desafios é onde a transformação precisa acontecer primeiro`
      );
    }
  }
...
export function deriveVitalidadeEnergia(
  ...
  const shadowSymptoms = buildShadowSymptoms(
    astro,
    kab,
    tantra,
    odu,
  ...
  const giftStrengths = buildGiftStrengths(
    astro,
    kab,
    tantra,
    odu,
  ...
  const dailyTransit = deriveDailyTransitOverlay(
    astro,
    kab,
    tantra,
    odu,
...
export function deriveMissaoDestino(
  ...
  const lifePathStr =
    kab?.lifePath && kab?.lifePathMaster
  ...
  const destinyStr = tantra?.destiny ? `Seu destino é: ${tantra.destiny}.` : '';
...
export function deriveDesafiosSombras(
  ...
  const karmicDebtsStr =
    kab?.karmicDebts && kab.karmicDebts.length > 0
      ? `Dívidas kármicas Cabalísticas: ${kab.karmicDebts.join(', ')}.`
      : '';

  const challengesStr =
    kab?.challenges?.first || kab?.challenges?.second
  ...
  return {
    ...
    pillarContribution: {
      ...
      tantra: tantra?.karma
        ? `Seu Carma Tântrico ${tantra.karma} indica padrões a serem transmutados nesta vida.`
        : '',
