...
// NOTE: PERSONAL_DAY_EXERCISES content is defined locally to avoid circular
// imports. The content is identical to the original in ../index.ts.
// Only edit in one place — prefer ../index.ts as the canonical source.
const PERSONAL_DAY_EXERCISES: Record<
  DayEnergy,
  Record<
    string,
    {
...
> = {
  ...
  spiritual: {
    vitalidadeEnergia: {
      ...
      instruction:
        'Peça um momento de silêncio expandido. Visualize-se conectado a algo maior — Deus, o cosmos, a natureza. Permita que o corpo se sinta parte de algo infinito.',
      duration: '15 minutos',
      difficulty: 'light',
      type: 'meditation',
    },
    ...
    carreiraProsperidade: {
      area: 'carreiraProsperidade',
      title: 'Missão Acima de Trabalho',
      instruction:
...
export function personalDayExercises(snapshot: PersonalCycleSnapshot): EvolutionaryExercise[] {
  ...
  return AREA_KEYS.map((areaKey): EvolutionaryExercise | null => {
...
export function personalMonthExercises(snapshot: PersonalCycleSnapshot): EvolutionaryExercise[] {
  ...
  const matchedArea =
    focusAreaMap[personalMonth.focus] ??
    AREA_KEYS[Math.floor(Date.now() / 1000) % AREA_KEYS.length];
