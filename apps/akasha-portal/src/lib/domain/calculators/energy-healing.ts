...
export function buildEnergyHealingMap(birthDate: string, fullName: string): EnergyHealingMap {
  ...
  reikiSymbols.push({
    ...
    purpose:
      'Amplificar a energia vital, selar o campo áurico de proteção e trazer aterramento na matéria.',
  ...
  if ([2, 3, 5, 6, 8, 9].includes(month) || day % 2 === 0) {
    reikiSymbols.push({
      ...
      purpose:
        'Harmonizar sentimentos de ansiedade, pânico, mágoas familiares ocultas e acalmar o Ori Quente.',
  ...
  if (karmicDebts.length > 0 || [4, 8, 12, 16, 20, 24, 28].includes(day)) {
    reikiSymbols.push({
      ...
      purpose:
        'Acessar registros do passado (infância e vidas passadas) para liberar bloqueios de falta de perdão familiar.',
  ...
  if ([7, 9, 11, 22].includes(reducedDay)) {
    reikiSymbols.push({
      ...
      purpose:
        'Despertar a conexão direta com a luz divina da alma, clareza sobre o propósito de vida e ativação do corpo sutil.',
  ...
  let groundingProtocol = {
    ...
    rationale:
      'Seu mapa não indica dívidas kármicas ativas. A prática atua na manutenção preventiva da paz e estabilização energética.',
  };
  ...
  if (karmicDebts.includes(13)) {
    groundingProtocol = {
      ...
      purpose:
        'Exercícios de respiração ativa e caminhadas conscientes na terra/grama para desbloquear a inércia e atrair prosperidade.',
      rationale:
        'A dívida kármica 13 indica memórias de estagnação e negligência em encarnações anteriores. O aterramento contínuo ajuda a reestabelecer o fluxo de ação e produtividade.',
    };
  } else if (karmicDebts.includes(14)) {
    groundingProtocol = {
      technique: 'Aterramento para Integração Emocional e Equilíbrio da Alma',
      purpose:
        'Respiração Pranayama e meditações de centramento com plantas medicinais ou chás calmantes.',
      rationale:
        'A dívida kármica 14 reflete abusos de liberdade pessoal no passado. A prática de aterramento com respiração ritmada devolve o autocontrole, centramento e a paz mental.',
    };
  } else if (karmicDebts.includes(16)) {
    groundingProtocol = {
      technique: 'Aterramento de Proteção e Fortalecimento do Templo Sagrado',
      purpose:
        'Visualizações de enraizamento profundo da coluna vertebral no núcleo da Terra, fortalecendo a aura.',
      rationale:
        'A dívida kármica 16 sinaliza a necessidade de reconstruir a base ética e a humildade. Este aterramento fortalece o Chakra Cardíaco e protege contra oscilações de ego e auto-obsessão.',
    };
  } else if (karmicDebts.includes(19)) {
    groundingProtocol = {
      technique: 'Aterramento Solar e Alinhamento de Poder Consciente',
      purpose:
        'Banhos de sol conscientes (amanhecer) em contato direto com a terra (pés descalços).',
      rationale:
        'A dívida kármica 19 exige o aprendizado de liderar com empatia. O aterramento solar equilibra o plexo solar, removendo a necessidade de controle e restaurando a liderança fraterna.',
    };
  }
