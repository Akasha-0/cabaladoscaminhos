...
export function buildForestMedicineMap(birthDate: string, fullName: string): ForestMedicineMap {
      ...
      recomendedRapes: [
        {
          ...
          indication:
            'Indicado para limpeza de pensamentos repetitivos e harmonização energética diária.',
        },
      ],
      spiritualWarning:
        'Mantenha a atenção plena nas suas relações cotidianas e cultive o perdão para evitar bloqueios de caminho.',
    };
  }
  ...
  if (personalYear === 9) {
    ayahuascaProtocol = {
      ...
      rationale:
        'O Ano Pessoal 9 é o tempo de encerramento, limpeza profunda, desapego e busca interior sagrada.',
    };
    ...
    ayahuascaProtocol = {
      ...
      rationale:
        'O Ano Pessoal 7 favorece fortemente o estudo interior, a meditação, o silêncio e a conexão espiritual.',
    };
    ...
    ayahuascaProtocol = {
      ...
      rationale:
        'O Ano Pessoal 5 traz mudanças rápidas e volatilidade; este protocolo apoia a maleabilidade mental e o foco.',
    };
  ...
  if (birthOdu === 9) {
    // Ossá (Ventos, instabilidade)
    rapes.push({
      name: 'Rapé Tsunu (Sabedoria da Floresta)',
      purpose: 'Aterramento, centramento e organização dos pensamentos acelerados',
      indication:
        'Indicado para dissipar ventos mentais e a nebulosidade emocional típica do Odu Ossá.',
    });
  } else if (birthOdu === 3 || birthOdu === 11) {
    // Etogundá / Owarin (Guerra, impeto, disputa)
    rapes.push({
      name: 'Rapé Canela de Velho',
      purpose: 'Harmonização do ímpeto agressivo e alívio de tensões de batalha',
      indication:
        'Indicado para acalmar a impulsividade, aplacar o estresse físico e restaurar a paz interior.',
    });
  } else if (birthOdu === 4 || birthOdu === 7) {
    // Irosun / Odi (Sangue, segredos, perigo oculto)
    rapes.push({
      name: 'Rapé Murici',
      purpose: 'Aterramento físico, presença absoluta e instinto de autoproteção',
      indication:
        'Atua no chakra básico para trazer segurança, firmeza física e limpar energias de inveja ou traição.',
    });
  } else if (birthOdu === 6) {
    // Obará (Fartura, prosperidade)
    rapes.push({
      ...
      indication:
        'Sintoniza a mente com a vibração de prosperidade e sabedoria material do Odu Obará.',
    });
  } else {
  ...
  let spiritualWarning =
    'Atenção às oscilações de pensamento acelerado. Cultive a paciência nas suas relações diárias.';
  ...
  if (isOriQuente) {
    spiritualWarning =
      'Vigilância contra o "Ori Quente" (cabeça quente/acelerada). O Odu natal indica propensão a estresse mental, ansiedade e panico quando desalinhado. Evite seguir caminhos impulsivos ou tóxicos para o seu destino. Pratique o aterramento diário com Rapé Tsunu ou Canela de Velho.';
  }
  ...
  if (hasFamilyKarmicChallenge) {
    spiritualWarning +=
      ' Há indicação de bloqueios kármicos associados a dinâmicas familiares (relação com a mãe ou com o pai). A falta de perdão e mágoas não resolvidas do passado atuam diretamente travando a fluidez dos seus caminhos materiais. Pratique o auto-acolhimento e busque a cura das raízes ancestrais.';
  }

  // Adicionar aviso de quizila do Odu
  if (birthOdu === 1) {
    spiritualWarning +=
      ' Quizila de Ogbe: evite pular etapas em projetos e nunca tome decisões importantes de forma apressada.';
  } else if (birthOdu === 6) {
    spiritualWarning +=
      ' Preceito de Obará: afaste-se da ganância e mantenha a humildade para que a prosperidade continue fluindo nos seus caminhos.';
  } else if (birthOdu === 8) {
    spiritualWarning +=
      ' Quizila de Ejionile: evite a arrogância ou o orgulho excessivo, e cultive a retidão e a justiça em suas palavras.';
  }
