...
- Name: api.spec.ts >> Protected APIs >> GET /api/creditos should return 401 without auth
- Location: e2e/api.spec.ts:78:7
...
Received value: {"error": "Usuário não autenticado"}
```
...
> 84  |     expect(body).toHaveProperty('success', false);
  ...
  150 |     // CORS headers may or may not be present depending on config
  151 |   });
  152 | });
```