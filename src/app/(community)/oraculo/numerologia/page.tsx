...
import { NumerologiaGrid } from '@/components/oraculo/NumerologiaGrid';
import { MapaNumerológico } from '@/lib/oraculo/numerologia';

export default function NumerologiaPage() {
  const [mapa, setMapa] = useState<MapaNumerológico | null>(null);
    ...
    try {
      const res = await fetch('/api/oraculo/numerologia', {
        ...
        body: JSON.stringify({
          ...
          sistema: data.sistemaNumerologia ?? 'pitagorica',
          anoReferência: new Date().getFullYear(),
        }),
      });
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-3xl px-4 py-6 md:px-6 md:py-10">
        ...
        <header className="mb-6 flex items-center gap-3">
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-purple-500/30 to-violet-500/20 ring-1 ring-purple-400/40">
            <Hash className="h-6 w-6 text-purple-300" aria-hidden />
          </div>
          <div>
            ...
            <p className="text-xs text-slate-400">
              Pitagórica · Caldeia · Cabalística · Algoritmos open-source
            </p>
          </div>
        </header>
        ...
        {/* Form OR Result */}
        {!mapa ? (
          <InputForm modo="numerologia" onSubmit={onSubmit} loading={loading} error={error} />
        ) : (
          <div className="flex flex-col gap-6">
            <NumerologiaGrid mapa={mapa} />
            <button
