...
// Filtros: tradição, type, date range. Toggle List/Calendar. Featured.
// ============================================================================

import React, { useState, useEffect, useMemo } from 'react';
import Link from 'next/link';
import {
  Calendar, Search, Loader2, Plus, Users, Globe,
...
export default function EventsBrowsePage() {
  ...
  const featured = useMemo(
    () => events.filter((e) => e.tags.includes('featured')).slice(0, 3),
    [events],
  );
            ...
            <div className="flex items-center gap-2 flex-wrap text-xs">
              ...
              <div className="ml-auto flex items-center gap-1 bg-slate-800/40 rounded-md p-0.5">
                <button
                  ...
                  className={cn('p-1.5 rounded text-xs', view === 'list' ? 'bg-slate-700 text-slate-200' : 'text-slate-500')}
                ...
                <button
                  ...
                  className={cn('p-1.5 rounded text-xs', view === 'calendar' ? 'bg-slate-700 text-slate-200' : 'text-slate-500')}
...
function EventCard({ event, featured = false }: { event: EventItem; featured?: boolean }) {
  ...
  return (
    <Link href={`/events-v2/${event.id}`} data-testid={`event-card-${event.id}`} className="block group">
      <Card className={cn('card-spiritual bg-gradient-to-br border transition-all h-full', colorClass, featured && 'ring-2 ring-amber-500/40')}>
        ...
        <CardContent className="pt-0 space-y-2">
          ...
          <div className="flex flex-col gap-1 text-xs text-slate-300">
            ...
            {event.priceCents > 0 && (
              <span className="flex items-center gap-1.5">R$ {(event.priceCents / 100).toFixed(2)}</span>
            )}
          </div>
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          {days.map((d) => {
            ...
            return (
              <div
                key={d.toISOString()}
                className={cn('aspect-square p-1 rounded border border-slate-800/50 text-left overflow-hidden', isToday && 'border-amber-500/50 bg-amber-500/5')}
              >
