...
const INITIAL: FormState = {
  type: '', tradition: '', title: '', description: '',
  startsAt: '', endsAt: '', timezone: 'America/Sao_Paulo',
  location: '', onlineUrl: '',
  capacity: '', priceCents: 0, rsvpRequired: true,
  coverImage: '', tags: [],
};
      ...
      <main className="max-w-3xl mx-auto space-y-6 pb-24">
        <header className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 mt-0.5">Workshop, círculo, palestra, cerimônia ou livestream</p>
          </div>
        </header>
              ...
              {STEPS.map((s) => {
                ...
                return (
                  <li key={s.num} className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {s.num < STEPS.length && <span className="text-slate-700">›</span>}
                  </li>
                );
              })}
            </ol>
        ...
        <Card className="card-spiritual bg-slate-900/50 border-slate-800/50">
          <CardContent className="pt-6 space-y-4">
            ...
            {step === 5 && (
              <Step5 form={form} update={update} tagInput={tagInput} setTagInput={setTagInput} />
            )}
          </CardContent>
        </Card>

        <div className="flex items-center justify-between gap-2">
          <Button variant="outline" onClick={back} disabled={step === 1 || submitting} className="border-slate-700 text-slate-300" data-testid="step-back">
            <ArrowLeft className="w-4 h-4 mr-1" /> Voltar
          </Button>
          <div className="flex items-center gap-2">
            {step === 5 && (
              <Button variant="outline" onClick={() => submit('DRAFT')} disabled={submitting} className="border-slate-700 text-slate-300" data-testid="save-draft">
                Salvar rascunho
              </Button>
            )}
            {step < 5 ? (
              <Button onClick={next} disabled={!validation.ok} className="bg-gradient-to-r from-amber-500 to-violet-500 text-white border-0" data-testid="step-next">
                Próximo <ArrowRight className="w-4 h-4 ml-1" />
              </Button>
            ) : (
              <Button onClick={() => submit('PUBLISHED')} disabled={submitting || !validation.ok} className="bg-gradient-to-r from-amber-500 to-violet-500 text-white border-0" data-testid="publish-event">
          ...
          {EVENT_TYPES.map((t) => (
            <button
              ...
              className={cn(
                ...
                form.type === t.value ? 'bg-amber-500/20 border-amber-500/50' : 'bg-slate-800/30 border-slate-700/50 hover:border-slate-600',
              )}
            >
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input type="datetime-local" value={form.startsAt} onChange={(e) => update('startsAt', e.target.value)} data-testid="starts-at-input"
            className="w-full px-3 py-2.5 rounded-xl bg-slate-800/50 border border-slate-700/50 focus:border-amber-500/50 text-sm text-slate-200 outline-none" />
        </div>
        <div>
          <label className="block text-xs text-slate-400 mb-2">Término *</label>
          <input type="datetime-local" value={form.endsAt} onChange={(e) => update('endsAt', e.target.value)} data-testid="ends-at-input"
            className="w-full px-3 py-2.5 rounded-xl bg-slate-800/50 border border-slate-700/50 focus:border-amber-500/50 text-sm text-slate-200 outline-none" />
        </div>
      </div>
      <div>
...
function Step3({ form, update }: { form: FormState; update: <K extends keyof FormState>(k: K, v: FormState[K]) => void }) {
  ...
  return (
    <div className="space-y-4">
      {physical && (
        <div>
          <label className="block text-xs text-slate-400 mb-2">Localização física</label>
          <input type="text" value={form.location} onChange={(e) => update('location', e.target.value)} maxLength={500}
            placeholder="Ex: Centro Akasha — R. das Tradições, 123 — São Paulo" data-testid="location-input"
            className="w-full px-3 py-2.5 rounded-xl bg-slate-800/50 border border-slate-700/50 focus:border-amber-500/50 text-sm text-slate-200 outline-none" />
        </div>
      )}
      {online && (
        <div>
          <label className="block text-xs text-slate-400 mb-2">URL online</label>
          <input type="url" value={form.onlineUrl} onChange={(e) => update('onlineUrl', e.target.value)}
            placeholder="https://meet.jit.si/circulo-cabala-2026" data-testid="online-url-input"
            className="w-full px-3 py-2.5 rounded-xl bg-slate-800/50 border border-slate-700/50 focus:border-amber-500/50 text-sm text-slate-200 outline-none" />
        </div>
      )}
      {!physical && !online && <p className="text-sm text-slate-400">Selecione um tipo de evento no passo 1.</p>}
    </div>
  );
}
...
function Step4({ form, update }: { form: FormState; update: <K extends keyof FormState>(k: K, v: FormState[K]) => void }) {
  return (
    <div className="space-y-4">
      <div>
        <label className="block text-xs text-slate-400 mb-2">Capacidade (vazio = ilimitado)</label>
        <input type="number" value={form.capacity} onChange={(e) => update('capacity', e.target.value)} min={1} max={10000} placeholder="Ex: 30" data-testid="capacity-input"
          className="w-full px-3 py-2.5 rounded-xl bg-slate-800/50 border border-slate-700/50 focus:border-amber-500/50 text-sm text-slate-200 outline-none" />
      </div>
      <div>
        <label className="block text-xs text-slate-400 mb-2">Preço (em centavos, 0 = gratuito)</label>
        <div className="flex items-center gap-2">
          <input type="number" value={form.priceCents} onChange={(e) => update('priceCents', parseInt(e.target.value || '0', 10))} min={0} data-testid="price-input"
      ...
      <div className="flex items-center gap-3 pt-2">
        <input type="checkbox" id="rsvpRequired" checked={form.rsvpRequired} onChange={(e) => update('rsvpRequired', e.target.checked)} data-testid="rsvp-required-input"
          className="w-4 h-4 rounded border-slate-700 bg-slate-800 text-amber-500 focus:ring-amber-500" />
        <label htmlFor="rsvpRequired" className="text-sm text-slate-300 cursor-pointer">Exigir RSVP para participar</label>
      </div>
    </div>
  );
}

function Step5({ form, update, tagInput, setTagInput }: { form: FormState; update: <K extends keyof FormState>(k: K, v: FormState[K]) => void; tagInput: string; setTagInput: (v: string) => void }) {
  const addTag = () => {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="block text-xs text-slate-400 mb-2">Imagem de capa (URL)</label>
        <input type="url" value={form.coverImage} onChange={(e) => update('coverImage', e.target.value)} placeholder="https://..." data-testid="cover-input"
          className="w-full px-3 py-2.5 rounded-xl bg-slate-800/50 border border-slate-700/50 focus:border-amber-500/50 text-sm text-slate-200 outline-none" />
      </div>
      <div>
        <label className="block text-xs text-slate-400 mb-2">Tags (até 10)</label>
        <div className="flex items-center gap-2">
          <input type="text" value={tagInput} onChange={(e) => setTagInput(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), addTag())} placeholder="Ex: iniciantes" data-testid="tag-input"
          ...
          <Button size="sm" onClick={addTag} variant="outline" className="border-slate-700">Adicionar</Button>
        </div>
        {form.tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-2">
            {form.tags.map((t) => (
              <Badge key={t} variant="outline" className="border-amber-500/30 text-amber-300 cursor-pointer hover:bg-red-500/10" onClick={() => removeTag(t)}>
                #{t} ×
              </Badge>
            ))}
          </div>
...
function validateStep(step: number, f: FormState): { ok: boolean; errors: string[] } {
  ...
  if (step === 2) {
    ...
    if (f.startsAt && f.endsAt && new Date(f.endsAt) <= new Date(f.startsAt)) errors.push('Término deve ser depois do início');
  }
  if (step === 3) {
    if (f.type !== 'LIVESTREAM' && !f.location) errors.push('Localização física obrigatória');
    if ((f.type === 'LIVESTREAM' || f.type === 'WORKSHOP') && !f.onlineUrl) errors.push('URL online obrigatória');
  }
  if (step === 4) {
    if (f.capacity && (parseInt(f.capacity, 10) < 1 || parseInt(f.capacity, 10) > 10000)) errors.push('Capacidade entre 1 e 10000');
