...
import {
  Calendar, MapPin, Globe, Users, DollarSign, Loader2,
  Check, X, ArrowLeft, Download, Share2, Video,
} from 'lucide-react';
...
export default function EventDetailPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`/api/community/events?limit=200`);
            ...
            <CardContent className="pt-6 space-y-4">
              <div className="flex items-center justify-between flex-wrap gap-2">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <Button size="sm" variant="outline" onClick={() => submitRsvp('MAYBE')} disabled={rsvpPending} className="border-slate-700">
                    Talvez
                  </Button>
                  {rsvpStatus && (
                    <Button
          ...
          <Card className="card-spiritual bg-slate-900/50 border-slate-800/50">
            ...
            <CardContent>
              <a href={event.recordingUrl} target="_blank" rel="noopener noreferrer">
                <Button variant="outline" className="border-slate-700">Assistir gravação</Button>
              </a>
            </CardContent>
          </Card>
        )}
