...
const AFFIRMATIONS: Affirmation[] = [
  ...
  { id: 'tar-005', texto: 'A Roda me ensina que cada ciclo traz aprendizado divino.', fonte: 'A Roda da Fortuna', categoria: 'tarot', tags: ['ciclos', 'aprendizado'] },
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (categoria) {
      const affirmation = getAffirmationByCategory(categoria, dayOfYear);
      if (!affirmation) {
        return NextResponse.json({
      ...
      return NextResponse.json({
        success: true,
        affirmation,
        meta: { diaDoAno: dayOfYear, data: now.toISOString() },
      });
    }

    return NextResponse.json({
      success: true,
      affirmation: getAffirmationOfDay(dayOfYear),
      categories: ['cabala', 'numerologia', 'orixas', 'tarot'],
