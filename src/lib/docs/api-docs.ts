 ...
 * Parses route files and generates structured API documentation
 */
 ...
 * Parses all files in src/app/api and returns structured documentation
 */
export async function generateAPIDocs(options: APIDocsOptions = {}): Promise<RouteDefinition[]> {
  ...
  if (options.includeAuth === false) {
    filtered = filtered.filter(r => !r.authRequired);
  }

  if (options.includeDeprecated === false) {
    filtered = filtered.filter(r => !r.path.includes('[deprecated]'));
  }
...
/**
 * Generate documentation as markdown string
 */
export async function generateAPIDocsMarkdown(options: APIDocsOptions = {}): Promise<string> {
  const routes = await generateAPIDocs(options);
  ...
  for (const [category, endpoints] of Object.entries(grouped)) {
    ...
    for (const route of endpoints) {
      markdown += formatRouteMarkdown(route);
    }
  }
...
async function parseRouteFile(
  ...
  try {
    ...
    const category = parts.length > 0 ? capitalize(parts[0]) : 'General';
...
function formatRouteMarkdown(route: RouteDefinition): string {
