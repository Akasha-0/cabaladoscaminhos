...
// ============================================================================
// WEBHOOK EVENT AUDIT LOG (Wave 33 — LGPD Art. 37)
// ============================================================================
// Persiste TODOS os eventos Stripe recebidos (incluindo duplicates e
// erros). Complementa ProcessedStripeEvent (que é só dedupe).
// Best-effort: falhas de log NÃO devem derrubar o webhook handler.
// ============================================================================

export interface PersistWebhookEventArgs {
  stripeId: string;
  type: string;
...
export async function persistWebhookEvent(
  args: PersistWebhookEventArgs
): Promise<void> {
  const prisma = await getPrisma();
  try {
    await prisma.webhookEvent.upsert({
      where: { stripeId: args.stripeId },
      create: {
        ...
        metadata: (args.metadata as object) ?? null,
        error: args.error ?? null,
        durationMs: args.durationMs,
        accountId: args.accountId ?? null,
        processedAt:
          args.processed && !args.duplicate ? new Date() : null,
      },
      update: {
        processed: args.processed,
        duplicate: args.duplicate,
    ...
    // Audit log é best-effort — não falhar webhook se log falhar
    console.error('[webhook] persistWebhookEvent falhou:', err);
  }
}
