 ...
 * Cabala dos Caminhos — wave 69, 2026-06-30 (B2 retry)
 *
 ...
 *            expectNotNull, expectThrows, expectMatches, expectLen, expectContains,
 *            expectDeepEqual, runXxxSpec().
...
export function expectFrozen(label: string, actual: unknown): void {
  assert(label, Object.isFrozen(actual), `expected frozen, got ${typeof actual}`);
}

export function expectGreaterThan(label: string, actual: number, threshold: number): void {
  assert(label, actual > threshold, `expected ${actual} > ${threshold}`);
}

export function expectLessThan(label: string, actual: number, threshold: number): void {
  assert(label, actual < threshold, `expected ${actual} < ${threshold}`);
}
