...
export const TAROT_TAROT_MAPPINGS: readonly TarotTarotMapping[] = [
  ...
  { arcano: 'II - A Sacerdotisa', arcano_numero: 2, arcano_romano: 'II', related_arcano: 'III - A Imperatriz', related_numero: 3, related_romano: 'III', path_type: 'Sequencia', significado: 'Misterio', crescimento: 'Aplicar', desafio: 'Integrar' },
...
export function getAllPathTypes(): TarotPathType[] {
  ...
  for (const m of TAROT_TAROT_MAPPINGS) {
    types.add(m.path_type);
  }
...
export function getPathTypeBetween(arcano1: string, arcano2: string): TarotPathType | null {
  for (const m of TAROT_TAROT_MAPPINGS) {
    if ((m.arcano === arcano1 && m.related_arcano === arcano2) || (m.arcano === arcano2 && m.related_arcano === arcano1)) {
      return m.path_type;
    }
  }
  return null;
...
export function getSpiritualMeaningBetween(arcano1: string, arcano2: string): { significado: string; crescimento: string; desafio: string } | null {
  for (const m of TAROT_TAROT_MAPPINGS) {
    if ((m.arcano === arcano1 && m.related_arcano === arcano2) || (m.arcano === arcano2 && m.related_arcano === arcano1)) {
      return { significado: m.significado, crescimento: m.crescimento, desafio: m.desafio };
    }
  }
  return null;
}
...
export default {
  getAllTarotPaths, getAllPathTypes, getAllMappedArcanos, getRelationsForArcano,
  getRelationsByPathType, getPathTypeBetween, getSpiritualMeaningBetween, hasRelation,
  getArcanoByNumber, getTarotTarot, ALL_MAJOR_ARCANOS, TAROT_TAROT_MAPPINGS, TOTAL_MAPPINGS, TOTAL_PATH_TYPES,
};