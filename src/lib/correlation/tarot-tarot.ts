...
const TAROT_TAROT_MAP: readonly TarotTarotMapping[] = [
  ...
  { arcano: '0 - O Louco', related_arcano: 'XXI - O Mundo', path_type: 'Ancestral', spiritual_meaning: { significado: 'Inicio e fim', crescimento: 'Ciclar jornada', desafio: 'Aceitar retorno' } },
];
...
export function getTarotTarot(arcano: string): TarotTarotMapping[] {
  return TAROT_TAROT_MAP.filter((m) => m.arcano === arcano || m.related_arcano === arcano);
}
...
export function getRelationsByPathType(type: TarotPathType): TarotTarotMapping[] { return TAROT_TAROT_MAP.filter((m) => m.path_type === type); }
