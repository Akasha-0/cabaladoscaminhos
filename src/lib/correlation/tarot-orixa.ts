/**
 * Tarot-Orixá Spiritual Correlation Module
 * Maps Tarot Major Arcana cards to Orixás based on spiritual energy and ritual correspondences
 * Reverse mapping of orixa-tarot.ts - Tarot as primary key
 */
 ...
 * This is the reverse mapping where Tarot is the primary key
 */
 ...
 * @param orixa - The Orixá name (e.g., 'Oxalá', 'Iemanjá')
...
/**
 * Get all Tarot arcano names in the mapping
...
/**
 * Check if a Tarot arcano exists in the mapping
 ...
 * @param arcano - Name of the Tarot Major Arcana card
 ...
 */
export function getArcanoElement(arcano: string): string | null {
  const mapping = getTarotOrixa(arcano);
  return mapping ? mapping.elemento : null;
}

/**
 * Get the Orixá for a given card number
...
/**
 * Get Tarot-Orixá mappings by element
 * @param elemento - The element to filter by
 * @returns Array of TarotOrixaMapping with the given element
 */
export function getTarotOrixasByElement(elemento: string): TarotOrixaMapping[] {
  return Object.values(TAROT_ORIXA_MAPPINGS)
    .filter(mapping => mapping.elemento.toLowerCase() === elemento.toLowerCase());
}
