...
/**
 * Represents the correlation between a Tarot Major Arcana card and an Orixá
 */
 ...
 * @param orixa - The Orixá name (e.g., 'Oxalá', 'Iemanjá', 'Ogum')
...
/**
 * Get the arcano number for a given Tarot card
 * @param arcano - Name of the Major Arcana card
 * @returns The arcano number or null if not found
 */
export function getTarotNumber(arcano: string): number | null {
...
/**
 * Get the Orixá name for a given Tarot card
 * @param arcano - Name of the Major Arcana card
 * @returns The Orixá name or null if not found
 */
export function getArcanoOrixa(arcano: string): string | null {
  const mapping = getTarotOrixa(arcano);
...
export function getOrixasByElement(elemento: string): string[] {
  return Object.entries(TAROT_ORIXA_MAPPINGS)
    .filter(([, mapping]) => mapping.elemento.toLowerCase() === elemento.toLowerCase())
