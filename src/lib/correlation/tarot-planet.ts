 ...
 * Reverse mapping of Planet-Tarot: Major Arcana to Classical Planets
 ...
 */
export function getPlanetFromTarot(arcano: string): string | null {
  const mapping = TAROT_PLANET_MAPPINGS[arcano];
 ...
 * @returns The planet name or null if not found
 */
export function getPlanetByArcanoNumber(numero: number): string | null {
  const found = Object.values(TAROT_PLANET_MAPPINGS).find(
 ...
 */
export function getArcanoByArcanoNumber(numero: number): string | null {
  const found = Object.values(TAROT_PLANET_MAPPINGS).find(
    mapping => mapping.numero_carta === numero
  );
