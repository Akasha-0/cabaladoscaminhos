 ...
 */
export function getTarotPlanetMapping(arcano: string): TarotPlanetMapping | null {
 ...
 */
export function getPlanetForArcano(arcano: string): string | null {
 ...
 */
export function getArcanoForPlanet(planeta: string): string | null {
 ...
 */
export function getAllTarotPlanetMappings(): TarotPlanetMapping[] {
 ...
 */
export function hasTarotPlanetMapping(arcano: string): boolean {
 ...
 * @returns The planet name or null if not found
 */
export function getPlanetByCardNumber(numero: number): string | null {
  for (const mapping of Object.values(TAROT_PLANET_MAPPINGS)) {
 ...
 * @returns The arcano name or null if not found
 */
export function getArcanoByCardNumber(numero: number): string | null {
  return TAROT_PLANET_MAPPINGS[Object.keys(TAROT_PLANET_MAPPINGS).find(key =>
    TAROT_PLANET_MAPPINGS[key].numero_carta === numero
  ) ?? '']?.arcano ?? null;
}

/**
 * Get tarot-planet mapping by card number
 * @param numero - The Major Arcana card number (1-21)
 * @returns The mapping or null if not found
 */
export function getMappingByCardNumber(numero: number): TarotPlanetMapping | null {
  for (const mapping of Object.values(TAROT_PLANET_MAPPINGS)) {
    if (mapping.numero_carta === numero) {
      return mapping;
    }
