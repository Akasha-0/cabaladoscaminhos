/**
 * Od'u-Ifá Orixá Correlation Module
 * Direct mappings of the 16 Odú Ifá (Merindilogun) to their ruling Orixás
...
export const ODTU_ORIXA_MAPPINGS: Record<number, OdTuOrixa> = {
  // ─── 1: Okaran (Okànràn) ───────────────────────────────────────────────────
  1: {
    odu_numero: 1,
    odu_nome: 'Okaran',
  ...
  3: {
    ...
    elemento: 'fogo',
    significado_espiritual: 'Força transformadora, criação,Tool-making. Poder de transformar matéria e criar vida através da vontade.',
  ...
  12: {
    ...
    orixa_aspect: 'Nanã',
    elemento: 'terra',
    significado_espiritual: 'Mistério, segredos sagrados, incognoscível. Reverência ao sagrado eaceitação do que não pode ser explicado.',
...
Object.freeze(ODTU_ORIXA_MAPPINGS);

// ─── Lookup Functions ────────────────────────────────────────────────────────

/**
 * Get Odu-Ifá Orixá correlation mapping
 * @param odu - Odu number (1-16) or name
 * @returns OdTuOrixa mapping or null if not found
 */
export function getOduOrixa(odu: number | string): OdTuOrixa | null {
  // Handle number input
  if (typeof odu === 'number') {
    if (odu >= 1 && odu <= 16) {
      return ODTU_ORIXA_MAPPINGS[odu] ?? null;
    }
    return null;
  }
...
/**
 * Get all Odu-Ifá Orixá mappings
 * @returns Array of all mappings sorted by Odu number
 */
export function getAllOduOrixas(): OdTuOrixa[] {
  return Object.values(ODTU_ORIXA_MAPPINGS).sort(
...
export function getAllOduNumbers(): number[] {
  return Object.keys(ODTU_ORIXA_MAPPINGS).map(Number).sort((a, b) => a - b);
}
...
export function getAllOduNames(): string[] {
  return Object.values(ODTU_ORIXA_MAPPINGS)
    .map((m) => m.odu_nome)
...
export function getAllOrixaNames(): string[] {
  ...
  Object.values(ODTU_ORIXA_MAPPINGS).forEach((m) => {
    names.add(m.orixa);
    if (m.orixa_aspect) {
 ...
 * @returns Array of OdTuOrixa mappings for that element
 */
export function getOduByElement(elemento: ElementType): OdTuOrixa[] {
  return Object.values(ODTU_ORIXA_MAPPINGS)
 ...
 * @returns Array of OdTuOrixa mappings for that Orixá
 */
export function getOduByOrixa(orixa: string): OdTuOrixa[] {
...
export function hasOduOrixa(oduNumero: number): boolean {
  return oduNumero in ODTU_ORIXA_MAPPINGS;
}
...
export function getOduElement(oduNumero: number): ElementType | null {
  return ODTU_ORIXA_MAPPINGS[oduNumero]?.elemento ?? null;
}
...
export function getOduMessage(oduNumero: number): string | null {
  return ODTU_ORIXA_MAPPINGS[oduNumero]?.mensagem_central ?? null;
}
