...
export const ODDU_SEPHIROT_MAPPINGS: Record<number, OdduSephirot> = {
  ...
  13: {
    ...
    elemento: 'terra',
    significado_espiritual:
      'Olobón (13): A doença, as transformações físicas, o fim de ciclos. O recolhimento. Este odú traz a energia da dissolução e do encerramento necessário. Representa o momento de recolhimento para cura e regeneração, quando é preciso parar e permitir a transformação.',
    conexoes_caminho: {
      numero_caminho: 9,
      letra_hebraica: 'ט',
      posicao: 'Fundação (Yesod)',
...
export function getOduSephirot(oddu: number | string): OdduSephirot | null {
  ...
  const found = Object.values(ODDU_SEPHIROT_MAPPINGS).find(
 ...
 * @returns Array of Oddu names
 */
export function getAllOdduNames(): string[] {
  return Object.values(ODDU_SEPHIROT_MAPPINGS)
    .map((m) => m.oddu_nome)
    .sort((a, b) => {
      const numA = ODDU_SEPHIROT_MAPPINGS[Object.values(ODDU_SEPHIROT_MAPPINGS).find((m) => m.oddu_nome === a)!.oddu_numero].oddu_numero;
      const numB = ODDU_SEPHIROT_MAPPINGS[Object.values(ODDU_SEPHIROT_MAPPINGS).find((m) => m.oddu_nome === b)!.oddu_numero].oddu_numero;
      return numA - numB;
    });
}
