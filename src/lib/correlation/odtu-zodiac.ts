...
export function getOduZodiac(odu: number | string): OdTuZodiac | null {
  if (typeof odu === 'number') {
    if (odu >= 1 && odu <= 16) {
      return ODTU_ZODIAC_MAPPINGS[odu] ?? null;
    }
    return null;
  }
  ...
  if (!isNaN(parsed) && parsed >= 1 && parsed <= 16) {
    return ODTU_ZODIAC_MAPPINGS[parsed] ?? null;
  }
  ...
  for (const mapping of Object.values(ODTU_ZODIAC_MAPPINGS)) {
    ...
    if (nomePT.includes(normalizedInput) || nomeYoruba.includes(normalizedInput)) {
      return mapping;
    }
  }
...
export function getAllOduZodiacs(): OdTuZodiac[] {
  return Object.values(ODTU_ZODIAC_MAPPINGS).sort(
...
export function getAllZodiacSigns(): ZodiacSign[] {
  ...
  Object.values(ODTU_ZODIAC_MAPPINGS).forEach((m) => {
    signs.add(m.signo);
  });
