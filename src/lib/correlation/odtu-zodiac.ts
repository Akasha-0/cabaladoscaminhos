...
export function getOduByElement(elemento: ElementType): OdTuZodiac[] {
  return Object.values(ODTU_ZODIAC_MAPPINGS).filter((m) => m.elemento === elemento).sort((a, b) => a.odu_numero - b.odu_numero);
}

export function getZodiacOdu(signo: string): OdTuZodiac[] {
  const normalized = signo.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '');
  return Object.values(ODTU_ZODIAC_MAPPINGS).filter((m) => m.signo.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '') === normalized).sort((a, b) => a.odu_numero - b.odu_numero);
}
...
export default { getOduZodiac, getZodiacOdu, getAllOduZodiacs, getAllOduNumbers, getAllOduNames, getAllZodiacSigns, getOduByElement, hasOduZodiac, getOduZodiacSign, getOduElement, getOduMessage };
