...
function calculateCorrelationStrength(
  ...
  for (const [orixa, planeta, corr] of knownCorrelations) {
    if ((entity1.toLowerCase().includes(orixa) && entity2.toLowerCase().includes(planeta)) ||
...
export class HyperCorrelationEngine {
  ...
  analyze(input: HyperCorrelationInput): CrossTraditionCorrelation {
    ...
    if (input.signoSolar) {
      ...
      correlations.push({
        ...
        value: element,
        strength: calculateCorrelationStrength('zodiac', input.signoSolar, 'orixa', primaryEntity.value),
  ...
  private determinePrimaryEntity(input: HyperCorrelationInput): CrossTraditionCorrelation['primaryEntity'] {
    if (input.orixaCabeca) {
  ...
  private generateInsights(
    ...
    if (input.caminhoVida === 11) {
      insights.push({
        level: 'high',
        description: 'Você carrega a vibração do MESTRE 11 — a intuição mais elevada da numerologia.',
  ...
  private calculateCycles(
    ...
    const birthMonth = input.dataNascimento ? 
      parseInt(input.dataNascimento.split('-')[1]) : 6;
