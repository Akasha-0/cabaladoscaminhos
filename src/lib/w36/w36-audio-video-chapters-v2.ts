/**
 * w36/audio-video-chapters-v2.ts
 *
 * PATCH of w36/audio-video-chapters.ts (cycle 36 audit fix #5).
 *
 * **Bug fixed (Verifier cycle 36 audit, non-blocking nit):**
 *   - `detectExplicitChapters` emitted `cueCount: i + 1 - i` for every
 *     detected chapter. This expression always evaluates to `1`, regardless
 *     of how many cues actually belong to the chapter. The intent was
 *     clearly the number of cues from this marker up to the next marker
 *     (or end of stream). v2 finds the next marker index and uses
 *     `(nextMarkerIdx - i)` as the correct count.
 *   - Related edge case: when the LAST cue of the stream is itself a marker,
 *     the original `endMs: endCue.startMs` (where `endCue = cues[i + 1] ?? cue`)
 *     produced a zero-length chapter (`endMs === startMs`). v2 falls back to
 *     the end-of-stream timestamp (`cues[cues.length - 1].endMs`) in this
 *     case.
 *
 * **Compatibility:** chapter objects now have correct `cueCount` and
 * (for the trailing-marker edge case) correct `endMs`. No type changes;
 * pure value-level fix.
 *
...
export function detectExplicitChapters(
  ...
  // Pre-compute the list of marker indices once, in order, to avoid an O(n^2)
  // re-scan of `cues` for every detected chapter.
  const markerIndices: number[] = [];
  for (let k = 0; k < cues.length; k++) {
    if (parseExplicitMarker(cues[k].text, pattern) !== null) {
      markerIndices.push(k);
    }
  }
  const lastCueEndMs =
    cues.length > 0 ? cues[cues.length - 1].endMs : 0;

  for (let m = 0; m < markerIndices.length; m++) {
    const i = markerIndices[m];
    const cue = cues[i];
    const marker = parseExplicitMarker(cue.text, pattern);
    if (marker === null) continue; // defensive — should not happen
    const nextMarkerIdx =
      m + 1 < markerIndices.length ? markerIndices[m + 1] : cues.length;
    const endCue = cues[nextMarkerIdx] ?? cue;
    // v2 fix: endMs falls back to the last cue's endMs for the trailing marker
    // edge case, so the final chapter has a positive duration rather than 0.
    const endMs = nextMarkerIdx < cues.length ? endCue.startMs : lastCueEndMs;
    const cueCount = nextMarkerIdx - i;
    const wordCount = countWordsInRange(cues, i, nextMarkerIdx);
    chapters.push({
