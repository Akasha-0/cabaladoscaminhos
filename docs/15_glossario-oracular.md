...
## 1. As 36 Cartas Ciganas (Lenormand) — Significado-Base Canônico

Cada entrada: **keywords** (do Doc 04 §5.1) + **significado-base** (frase-verdade injetável) + **sombra** (face desafiadora, para o Oráculo dosar o aviso com proteção).
...
> Estrutura sugerida para `src/lib/constants/lenormand-cards.ts`: adicionar campos `baseMeaning` e `shadow` a cada entrada (além de `keywords`).
