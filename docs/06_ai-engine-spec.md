...
O motor de IA não é um chatbot genérico. É um **motor de inferência determinístico** que segue uma cadeia de raciocínio rígida para cada uma das 36 casas. **No motor de geração do dossiê (por-casa), a IA nunca recebe perguntas abertas** — ela recebe dados estruturados e retorna análises formatadas.

> **Resolve I3 do Doc 10.** A regra "nunca perguntas abertas" aplica-se **a este motor por-casa**. Existe uma **segunda camada** — o Motor de Consulta Interativa (Q&A, Doc 12) — onde perguntas abertas são aceitas, mas **roteadas deterministicamente** para casas e aspectos natais antes de chegar ao LLM. O determinismo é preservado em ambas as camadas: a pergunta aberta nunca vira "conhecimento livre", e sim um recorte estruturado do que já foi calculado.
...
// AS 36 ENTRADAS COMPLETAS (resolve G4 do Doc 10), derivadas da Seção 2 e do Doc 09 §3.
...
> **Nota de extensibilidade (Doc 14):** o tipo `CorrelationEntry` pode ganhar um bloco opcional por novo sistema oracular (ex.: `iching?`) sem migrar as 36 entradas existentes. Ver Doc 14 §2, passo 3.
...
// Resolve I6 do Doc 10: nomes de modelo configuráveis por env, com defaults.
const OPENAI_MODEL    = process.env.OPENAI_MODEL    ?? 'gpt-4o';
const ANTHROPIC_MODEL = process.env.ANTHROPIC_MODEL ?? 'claude-sonnet-4-6'; // Claude é o fallback (Doc 09 §4)
...
> **Camada de Q&A (Doc 12):** o mesmo `llm-client.ts` é reutilizado pelo endpoint `POST /api/consult`. A diferença é o payload (recorte RAG fechado roteado por tema) e a persona de consulta (Doc 12 §6) — o motor por-casa do dossiê permanece intacto.
